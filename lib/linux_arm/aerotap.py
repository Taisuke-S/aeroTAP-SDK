...
def IsConnected():
    ...
    # Linux
    lib.checkDevice.restype = ctypes.c_bool
    lib.checkDevice.argtypes = [ctypes.c_void_p]
    if  not lib.checkDevice(obj):
	    return False;
    return True;
...
def Init():
    ...
    # Linux
    lib.setFPS.argtypes = [ctypes.c_int]
    lib.setFPS.argtypes = [ctypes.c_void_p,ctypes.c_int]
    lib.setFPS(obj,camFPS)

    lib.useMJPG.argtypes = [ctypes.c_bool]
    lib.useMJPG.argtypes = [ctypes.c_void_p,ctypes.c_bool]
    lib.useMJPG(obj,bEnableMJPEG)

    if nIsUSB30 ==1:
        lib.setUSB20.argtypes = [ctypes.c_bool]
        lib.etUSB20.argtypes = [ctypes.c_void_p,ctypes.c_bool]
        lib.setUSB20(obj,True)

    lib.setFilter.argtypes = [ctypes.c_void_p,ctypes.c_int]
...
def StartCam(width,height):
    ...
    if os.name == "nt":
        ...
        return True
    # Linux
    lib.open.restype = ctypes.c_bool
    lib.open.argtypes = [ctypes.c_void_p, ctypes.c_char_p,ctypes.c_char_p,ctypes.c_int,ctypes.c_int]
    if not lib.open(obj,None, None, camWidth, camHeight):
        return False

    lib.start.restype = ctypes.c_bool
    lib.start.argtypes = [ctypes.c_void_p]
    if not lib.start(obj):
        return False
...
#    print(zdTable)
    ...
    # Linux
    lib.getZDTable.restype = ctypes.c_int
    lib.getZDTable.argtypes = [ctypes.c_void_p,ctypes.POINTER(ctypes.c_int32)]
    zdTableLen = lib.getZDTable(obj,zdTable)
    return zdTableLen
...
def GetPType():
    ...
    lib.getPType.restype = ctypes.c_int
    lib.getPType.argtypes = [ctypes.c_void_p]
    return lib.getPType(obj)
...
def IsUSB30():
    ...
    lib.getUSB20.restype = ctypes.c_bool
    lib.getUSB20.argtypes = [ctypes.c_void_p]
    bUSB20 = lib.getUSB20(obj)
    if bUSB20:
        return 1
    return 2
...
def IsNewFrame():
    ...
    if os.name == "nt":
        ...
        return False
    # linux
    lib.isNewFrame.restype = ctypes.c_bool
    lib.isNewFrame.argtypes = [ctypes.c_void_p]
    if lib.isNewFrame(obj):
        return True
    return False
...
def GetLastError():
    ...
    # Linux
    lib.isConnectionLost.restype = ctypes.c_bool
    lib.isConnectionLost.argtypes = [ctypes.c_void_p]
    if lib.isConnectionLost(obj):
        return 1
    return 0
...
#    typedef BOOL (__cdecl *AERO_GetImage)(BITMAPINFOHEADER * pBuffer,int nType,int *nMax/*=NULL*/, int *nTotal/*=NULL*/,int *nCount/*=NULL*/, int *nFrame/*=NULL*/, int *nP/*=NULL*/, int *nBlackout/*=NULL*/);
     ...
     # read bmih
        ...
        if mode==0:
            lib.getColorData.restype = ctypes.POINTER(ctypes.c_uint8)
            lib.getColorData.argtypes = [ctypes.c_void_p]
            deserialized = np.reshape(np.ctypeslib.as_array( lib.getColorData(obj),shape=(1,camWidth*camHeight*3)), newshape=(camHeight, camWidth,3))
            deserialized = deserialized[:,:,::-1] # to BGR
        else:
            lib.getGrayData.restype = ctypes.POINTER(ctypes.c_uint8)
            lib.getGrayData.argtypes = [ctypes.c_void_p]
            deserialized = np.reshape(np.ctypeslib.as_array( lib.getGrayData(obj),shape=(1,camWidth*camHeight)), newshape=(camHeight, camWidth,1))
...
def ReadDepth():
    ...
    if not os.name == "nt":
    # Linux
        lib.getDepthData.restype = ctypes.POINTER(ctypes.c_uint16)
        lib.getDepthData.argtypes = [ctypes.c_void_p]
        depth = np.frombuffer(np.ctypeslib.as_array(lib.getDepthData(obj),shape=(1,camWidth*camHeight)),dtype=np.uint16)
        return depth
...
# Linex
        lib.updateFrame.argtypes = [ctypes.c_void_p]
        return lib.updateFrame(obj)
...
# Linex
    lib.enablePalmTracker.argtypes = [ctypes.c_void_p,ctypes.c_bool]
    lib.enablePalmTracker(obj,mode)
    return None
...
# Linex
    lib.setPalmDetectMode.argtypes = [ctypes.c_void_p,ctypes.c_int]
    lib.setPalmDetectMode(obj,mode)
    return None
...
# Linex
    lib.doPalmTracking.argtypes = [ctypes.c_void_p]
    lib.doPalmTracking(obj)
    return None
...
def getPalmTrackResult():
    ...
    if os.name == "nt":
        ...
        cHandResult.used = hr[9]
        cHandResult.centerX = hr[10]
        cHandResult.centerY = hr[11]
        cHandResult.centerZ = hr[12]
        if cHandResult.used:
            print("Hand detect!", cHandResult.centerX,cHandResult.centerY,cHandResult.centerZ)
            return True
        return False
   
# Linex
    HANDRESULT = struct.Struct("=llllIiiiiqqii")
    sz = 40*2 # struct.calcsize(HANDRESULT)
    buffer = np.empty(sz*2,dtype=np.uint8)
    lib.getPalmTrackResult.argtypes = [ctypes.c_void_p,ctypes.POINTER(ctypes.c_uint8)]
    if lib.getPalmTrackResult(obj,buffer.ctypes.data_as(ctypes.POINTER(ctypes.c_uint8))):
        hr = HANDRESULT.unpack(buffer[:sz])
        cHandResult.rect_x = hr[0]
        cHandResult.rect_y = hr[1]
        cHandResult.rect_width = hr[2]
        cHandResult.rect_heiht = hr[3]
        cHandResult.used = hr[4]
